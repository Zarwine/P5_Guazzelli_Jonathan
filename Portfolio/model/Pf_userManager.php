...
class Pf_userManager extends DatabaseObject //Traite toute la partie utilisateur du site.
{
    ...
    public function verifName($userData) {
        $req = $this->bdd->prepare('SELECT id FROM pf_users WHERE username =?');
    ...
    public function verifEmail($userData) {
        $req = $this->bdd->prepare('SELECT id FROM pf_users WHERE email =?');
    ...
    public function addMember($userData){ //Ajoute l'utilisateur a la BDD et attend confirmation mail
        $req = $this->bdd->prepare("INSERT INTO pf_users SET username = ?, password = ?, email = ?, confirmation_token = ?");
    ...
    public function verifToken($params){ //vérifie token mail = token de BDD
        $user_id = $params["id"];
        $token = $params["token"];
        $req = $this->bdd->prepare('SELECT * FROM pf_users WHERE id = ?');
        $req->execute([$user_id]);
        $user = $req->fetch();                
        session_start(); 
        if($user && $user->confirmation_token == $token) {                
            $this->bdd->prepare('UPDATE pf_users SET confirmation_token = NULL, confirmed_at = NOW() WHERE id =?')->execute([$user_id]);
            $_SESSION['auth'] = $user;
    ...
    public function login($userData){        
        $req = $this->bdd->prepare('SELECT * FROM pf_users WHERE (username = :username OR email = :username) AND confirmed_at IS NOT NULL');
    ...
    public function loginFail($ip){
        ...
        // On enregistre la tentative échouée pour cette ip
			$req = $this->bdd->prepare('INSERT INTO pf_bruteforce SET connexion_ip = :ip');
            $req->bindValue(':ip', $ip, PDO::PARAM_STR); 
            $req->execute();
    }
    public function loginFailCount($ip){
                // On regarde s'il est autorisé à se connecter
            $req = $this->bdd->prepare('SELECT * FROM pf_bruteforce WHERE connexion_ip = :ip');
            $req->bindValue(':ip', $ip, PDO::PARAM_STR); 
            $req->execute();
            $count = $req->rowCount();
            return $count;
    ...
    public function forgetPassword($userData){
        $req = $this->bdd->prepare('SELECT * FROM pf_users WHERE email = ? AND confirmed_at IS NOT NULL');
        $req->execute([$userData['email']]);
        $user = $req->fetch();
        return $user;
    }
    ...
    $this->bdd->prepare('UPDATE pf_users SET reset_token = ?, reset_at = NOW() WHERE id = ?')->execute([$reset_token, $user->id]);
    }
    ...
    public function resetPassword($params){      //génère un nouveau MDP pour l'accés au compte utilisateur  
        ...
        $req = $this->bdd->prepare("SELECT id FROM pf_users WHERE reset_token = '$user_verif_reset_token'");
        $req->execute();
        $user_id = $req->fetch();
        if($user_id == false) {
            session_start();
        ...
        $req2 = $this->bdd->prepare("SELECT reset_token FROM pf_users WHERE id = $user_id->id");
        $req2->execute();
        ...
        if($user_id->id == $user_verif_id || $user_token_reset == $user_verif_reset_token){
            ...
            $req3 = $this->bdd->prepare('UPDATE pf_users SET password = :pwd, reset_at = NULL, reset_token = NULL WHERE id = :id');
            ...
            header('Location: https://jogu.fr/login');
        }else{
            $_SESSION['flash']['danger'] = "Une erreur est survenue";
            header('Location: https://jogu.fr/home');
        }
    ...
    public function changePassword($userData){        
            ...
            $password = password_hash($userData['password'], PASSWORD_BCRYPT);            
            $req = $this->bdd->prepare('UPDATE pf_users SET password = ? WHERE id = ?');
